...
class Vehicle(pygame.sprite.Sprite):
    def __init__(self, lane, vehicleClass, direction_number, direction, will_turn):
        ...
        self.originalImage = pygame.imazge.load(path)
...
class Main:
    ...
    while True:
        ...
        # Display the number of waiting vehicles at each signal
        for i in range(noOfSignals):
            direction = directionNumbers[i]  # Get the direction string ('right', 'left', 'up', 'down')
            if direction in vehicles:  # Ensure the direction exists
                count = 0
                for lane in vehicles[direction]:
                    if isinstance(vehicles[direction][lane], list):  # Only process lists
                        count += len(vehicles[direction][lane])  # Add to total count\

                # print(f"Total waiting vehicles in {direction}: {count}")
                waiting_vehicles = count - vehicles[direction]['crossed']
            else:
                waiting_vehicles = 0  # Default if no vehicles are present

            waiting_color = (255, 0, 0) if waiting_vehicles > 10 else (0, 255, 0)  # Red if congestion is high, else Green
            waiting_text = font.render(f"{waiting_vehicles}", True, waiting_color, white)
            screen.blit(waiting_text, (vehicleCountCoods[i][0], vehicleCountCoods[i][1] + 30))  # Offset position
